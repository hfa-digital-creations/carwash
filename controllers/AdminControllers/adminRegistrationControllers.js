...
import { generateAndSaveTokens } from "../../utils/tokenUtils.js";
...
export const registerAdmin = async (req, res) => {
  try {
    ...
    let adminRole = "admin";
    if (role === "superadmin" && req.admin.role === "superadmin") adminRole = "superadmin";
    else if (role === "superadmin") return res.status(403).json({ message: "Only SuperAdmin can create SuperAdmin" });

    const admin = await Admin.create({ fullName, email: email.toLowerCase(), password, role: adminRole });
...
export const loginAdmin = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: "Login successful", admin: { id: admin._id, fullName: admin.fullName, email: admin.email, role: admin.role }, accessToken, refreshToken });
  } catch (err) {
    res.status(500).json({ message: "Server error", error: err.message });
  }
};
...
export const getAllAdmins = async (req, res) => {
  ...
  res.status(200).json({ message: "Admins fetched", count: admins.length, admins });
};
